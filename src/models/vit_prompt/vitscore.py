...
class PromptedTransformer(Transformer):
    ...
    def incorporate_prompt1(self, x):
        # combine prompt embeddings with image-patch embeddings
        B = x.shape[0]
        # after CLS token, all before image patches
        x = self.embeddings(x)  # (batch_size, 1 + n_patches, hidden_dim) # image patches embeddings
        x = torch.cat((
                x[:, :1, :],
    ...
    def incorporate_prompt(self, x):
        ...
        # print('(x.shape[1]+1)//2', (x.shape[1])//2)
        score_embeddings = nn.Parameter(torch.tensor(x[:, (x.shape[1]+1)//2: , :]))
        # print('score embeddings',score_embeddings.shape, 'image patch', x[:, 1 : (x.shape[1]+1)//2, :].shape)
        x = torch.cat((
                x[:, :1, :],
                self.prompt_dropout(self.prompt_proj(self.prompt_embeddings + score_embeddings).expand(B, -1, -1)),
                x[:, 1 : (x.shape[1]+1)//2, :]
            ), dim=1)
        # (batch_size, cls_token + n_prompt(prompt+segscore) + n_patches, hidden_dim)
    ...
    def forward_deep_prompt1(self, embedding_output):
        attn_weights = []
        hidden_states = None
        weights = None
