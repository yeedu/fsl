...
class PromptedTransformer(Transformer):
    ...
    def incorporate_promptscore(self, x):
        # combine prompt embeddings with image-patch embeddings
        B = x.shape[0]
        print('prompt x shape', x.shape)
        # after CLS token, all before image patches
        x = self.embeddings(x)  # (batch_size, 1 + n_patches, hidden_dim) # image patches embeddings
        x = torch.cat((
                x[:, :1, :],
                self.prompt_dropout(self.prompt_proj(self.prompt_embeddings + x[:, 1: x.shape[1]/2, :]).expand(B, -1, -1)),
                x[:, x.shape[1]/2:, :]
            ), dim=1)
    ...
    def forward_deep_promptseg(self, embedding_output):
        attn_weights = []
        hidden_states = None
        weights = None
        B = embedding_output.shape[0]
...
class PromptedVisionTransformer(VisionTransformer):
    ...
    def forward(self, x, vis=False):
        x, attn_weights = self.transformer(x) # encoded, attn_weights
