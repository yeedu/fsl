...
class Embeddings(nn.Module):
    ...
    def forward_seg1(self, x):
        B = x.shape[0]
        cls_tokens = self.cls_token.expand(B, -1, -1)
        print(x.shape)
        if self.hybrid:
            x = self.hybrid_model(x[0])
        x = self.patch_embeddings(x[0]) # input image
        x_seg = self.patch_embeddings(x[1]) # segmentated mask
        x = x.flatten(2)
        x = x.transpose(-1, -2)
        x_seg = x_seg.flatten(2)
        x_seg = x_seg.transpose(-1, -2)
        x = torch.cat((cls_tokens, x, x_seg), dim=1)

        embeddings = x + self.position_embeddings
        embeddings = self.dropout(embeddings)
        return embeddings

    def forward_seg2(self, x):
        B = x.shape[0]
        cls_tokens = self.cls_token.expand(B, -1, -1)
