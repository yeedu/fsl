...
class Evaluator():
    ...
    def _eval_singlelabel(
        ...
        # print('scores---------------------------------------------------', scores)
        # scores  = np.argmax(scores, axis=1)# np.array(torch.argmax(torch.tensor(scores), dim=1)) #.data
        # # print('scores---------------------------------------------------',scores)
        arr = confusion_matrix(y_pred=np.argmax(scores, axis=1), y_true=targets)
        print(arr)
        # df_cm = pd.DataFrame(arr, columns=['Positive','Negative'],index = ['Positive','Negative'])
        # print(df_cm)
        log_results = {
