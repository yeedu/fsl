...
class Trainer():
    ...
    def train_classifier(self, train_loader, val_loader, test_loader):
        ...
        for epoch in range(total_epoch):
            ...
            #         labels = X["label"]
            #         texts = X["txt"]
            #         img_feature = self.model.get_features(images.to(self.device)) # extract img_feature
            #         similarity = self.similarity(img_feature, texts)
            #         loss1 += similarity.cpu().detach().numpy().item()
                
            for idx, input_data in enumerate(train_loader):
                # print('idx', idx, input_data["image"].shape, input_data["label"].shape)
            ...
            # prompt_embds = self.model.enc.transformer.prompt_embeddings.detach().cpu().numpy()
            # print('prompt embds', prompt_embds.shape)
            # print('-----------------\n', prompt_embds)
    ...
    def similarity(self, img_feature, texts):
        ...
        input_ids = tokenizer.encode(texts, add_special_tokens=False)        
        ...
        text_feature = outputs[0][:, 0, :] # encoded_text['input_ids'] # 取第一个token的特征作为整个文本的特征
        
        # Getting Image and Text Embeddings (with same dimension)
        image_embeddings = ProjectionHead(embedding_dim=768).to(self.device)(img_feature.to(self.device))
        text_embeddings = ProjectionHead(embedding_dim=768).to(self.device)(text_feature.float().to(self.device))
