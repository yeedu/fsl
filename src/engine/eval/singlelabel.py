...
from sklearn.metrics import (
    accuracy_score, average_precision_score, f1_score, roc_auc_score,precision_score
)
...
def compute_acc_auc(y_probs, y_true_ids):
    ...
    if num_classes == 2:
        ...
        p = precision_score(y_true=y_true_ids, y_pred=np.argmax(y_probs, axis=1))
        auc1 = roc_auc_score(y_true_ids, np.argmax(y_probs, axis=1))
        ...
        try:
            ...
            auc = -1
        return {"top1": top1, "rocauc": auc, "auc1": auc1, "p": p} #{"AUC": auc, "ACC": top1}
